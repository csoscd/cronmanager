...
- **`Access denied` when applying database schema** – `docker exec mariadb` connects via Unix socket, which MariaDB resolves to `@'localhost'`. The Docker image creates the app user as `cronmanager@'%'`, and `'%'` does not match socket-based `@'localhost'` connections in MariaDB. Fixed by adding `-h 127.0.0.1` to both the `schema.sql` and migration `mariadb` commands, forcing TCP where the `@'%'` grant is honoured.

...
/**
 * Cronmanager Web UI – Application Bootstrap
 *
 * Singleton that initialises and exposes the Noodlehaus\Config instance and
 * the Monolog\Logger for the entire web application.
 *
...
final class Bootstrap
{
     ...
     *
     * Checks for the production config first; falls back to the local
     * development config so developers can work without the Docker volume.
     *
    ...
    {
        // Production path (Docker volume mount)
        if (file_exists(self::CONFIG_PATH_PRODUCTION)) {
            return self::CONFIG_PATH_PRODUCTION;
        }

        // __DIR__ is web/src – go up two levels to reach the web root,
        // then look for config/config.json
        $devConfig = dirname(__DIR__) . '/config/config.json';

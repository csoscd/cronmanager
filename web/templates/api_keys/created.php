...
<div class="max-w-2xl">
    ...
    <div class="mb-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-xl p-4 flex gap-3">
        ...
        <div class="text-sm text-amber-800 dark:text-amber-300">
            <strong><?= $h($t('api_key_one_time_warning')) ?></strong>
            <?= $h($t('api_key_one_time_hint')) ?>
        </div>
    </div>
    ...
    <div class="bg-gray-900 dark:bg-gray-950 rounded-xl p-5 mb-6 flex items-center gap-3">
        ...
        <button id="copy-btn" onclick="copyKey()"
                data-label-copy="<?= $h($t('api_key_copy')) ?>"
                data-label-copied="<?= $h($t('api_key_copied')) ?>"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 mb-6">
        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3"><?= $h($t('api_key_usage')) ?></p>
...
function copyKey() {
    ...
    navigator.clipboard.writeText(val).then(() => {
        if (btn) {
            ...
            setTimeout(() => {
                btn.innerHTML = `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg> ${labelCopy}`;
            }, 2000);
        }
    });
}

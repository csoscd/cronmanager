...
$formAction = isset($formAction) ? (string) $formAction : '/targets';
...
<nav class="mb-4 text-sm text-gray-500 dark:text-gray-400">
    <a href="/targets" class="hover:text-gray-700 dark:hover:text-gray-200">
        <?= $h($t('nav_targets')) ?>
    </a>
...
<div class="max-w-xl">
    ...
    <form method="POST" action="<?= $h($formAction) ?>" class="space-y-5">
        ...
        <!-- Target (read-only when embedded in target-specific URL, editable on edit) -->
        <div>
            ...
            <?php else: ?>
                <input type="text"
                       ...
                       value="<?= $h($target) ?>"
                       readonly
                       class="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-3 py-2 text-sm cursor-not-allowed">
            <?php endif; ?>
        </div>
